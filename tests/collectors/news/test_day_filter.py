...
def test_rss_timestamp_has_utc_timezone():
    item = parse_rss(b"<rss><channel><item><link>https://example.com/1</link>"
                     b"<pubDate>Mon, 20 Apr 2026 00:00:00 +0900</pubDate>"
                     b"</item></channel></rss>")[0]
...
def test_day_boundaries_and_limit_after_filter(monkeypatch):
    ...
    monkeypatch.setattr(news.Portfolio, "load", lambda root: SimpleNamespace(scope_tickers=lambda: []))
