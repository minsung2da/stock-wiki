...
from db.explorer import Explorer, Filters, make_server


@pytest.fixture
def explorer(pg_clean):
    with pg_clean.begin() as conn:
        conn.execute(text("INSERT INTO entities (corp_code, canonical_name, current_ticker) "
                          "VALUES ('00126380', '삼성전자', '005930')"))
        conn.execute(text("INSERT INTO news (url_hash, url, outlet, tickers, published_at, "
                          "title, content_hash, body_md) VALUES "
                          "(:hash, :url, '테스트', ARRAY['005930'], :published, :title, "
                          ":hash, :body)"), [
            dict(hash=str(i)*64, url=f'https://example.com/{i}', published=stamp,
                 title=title, body='저장된 본문 <script>alert(1)</script>\n' + '가'*5000)
        ...
        conn.execute(text("INSERT INTO fundamentals "
...
@pytest.mark.parametrize('values', [
    {'dataset': 'news;DROP TABLE news'}, {'sql': 'SELECT 1'}, {'page': '0'},
    {'page_size': '101'}, {'start': '2026-02-30'},
    {'start': '2026-09-24', 'end': '2026-09-23'}, {'ticker': '12'},
])
...
def test_inventory_and_kst_filters(explorer):
    ...
    filters = Filters(dataset='news', q='삼성전자', ticker='005930',
                      start='2026-09-23', end='2026-09-23', page_size=1)
...
def test_http_routes_and_boundary(explorer):
